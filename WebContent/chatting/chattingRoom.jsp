...
<body onbeforeunload="closePage()">
	<div>
		...
		%>
		<span><input type="text" id="title" value="<%=title%>"
			readonly="readonly"></span>

	</div>
	<div>
		<span><input type="text" id="nationality"
			...
			id="studytime" value="${param.studytime}" readonly="readonly">분</span>
		<span><input type="text" id="studypoint" value="${param.studypoint}"
			readonly="readonly"></span>
	</div>

	<fieldset>
		<textarea id="messageWindow" rows="35" cols="55" readonly="readonly"></textarea>
		<br /> <input id="inputMessage" type="text" /> <input type="submit"
			value="send" onclick="send()" />
	</fieldset>
	<div>
		<span id="countTimeMinute"></span>분 <span id="acountTimeSecond"></span>초
		<button id="start">커뮤니케이션 시작</button>
		<button id="end">커뮤니케이션 종료</button>
	</div>
	...
	<script type="text/javascript">
		...
		function onMessage(event) {
			...
			if (event.data == "&") {
				...
				textarea.value += "커뮤니케이션의 대가로 "+minute*divinedstudypoint+"포인트가 지불되었습니다."+"\n";
